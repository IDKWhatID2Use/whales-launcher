 ...
 *
 * 冻结契约要求：所有跨 IPC 的调用返回 `Result<T>`，异常绝不穿透。
 * IPC 层捕获到任何异常后都必须经 `describeError()` 转成可展示文案，
 * 因此这里承担了「错误消息质量」的全部责任。
 */

/** 消息可直接展示给用户的错误；主进程做前置校验时一律抛它。 */
...
const PATTERN_HINTS: ReadonlyArray<readonly [RegExp, string]> = [
  [
    /StartupError/i,
...
const CODE_HINT_SUPPRESSORS: ReadonlyArray<RegExp> = [/spawn .* ENOENT|not recognized as an internal/i];
 ...
 * @param error 捕获到的任何东西（Error / 字符串 / 未知对象）
 * @param fallback 完全无法提取信息时的兜底文案
 */
