...
这是本启动器最需要理解的一章。默认的「完全隔离」永远安全；一旦打开共享，就要知道代价。

---

## 1. 四个维度，各自独立决定

创建实例向导的第 4 步，以及实例详情的「隔离策略」卡片（存档页 / 设置页），都是这四个维度：
...
*图 10：向导第 4 步。四个维度各一行开关，顶部三个快速预设。*

### 三个快速预设

| 预设 | 效果 | 什么时候用 |
|---|---|---|
| **完全隔离（推荐）** | 四项全独立 | 绝大多数情况。默认就该选它 |
| **存档互通** | 只有会话存档共享 | 想让多个实例看到彼此的会话记录 |
| **全部共享** | 能共享的都共享 | 明确知道自己在做什么时才用 |
...
| 你删除了实例 | 结果 |
|---|---|
| 实例自己的 `home/`、`logs/` | 随实例目录删除（如果勾选了删除文件） |
...
- 按钮用 `btn--danger-solid`（危险实心）样式；
- 弹**二次确认**，确认框里写明「**解除链接不递归删除真实目录**」；
- 切换完成后，界面状态（左栏、卡片、页签）**立即同步**。
