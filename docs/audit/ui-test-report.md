...
生成时间：2026-09-22T04:57:24.458Z　耗时：146.5s
...
|---|---|---|---|---|---|---|
| 外壳（标题栏 / 左栏 / 应用菜单 / 日志抽屉 / 主题） |  | 11/11 | 0 | 0 | 1 | 18786ms |
| P1 实例列表 |  | 12/12 | 0 | 0 | 1 | 21102ms |
| P2 实例详情 · 插件 |  | 8/8 | 0 | 0 | 1 | 10958ms |
| P3 实例详情 · 设置 |  | 8/8 | 0 | 0 | 1 | 6524ms |
| P4 实例详情 · 存档 |  | 6/6 | 0 | 0 | 1 | 19893ms |
| P5 实例详情 · 日志 |  | 6/6 | 0 | 0 | 1 | 15183ms |
| P6 引擎版本管理 |  | 6/7 | 0 | 1 | 1 | 30202ms |
| P7 创建实例向导 |  | 8/8 | 0 | 0 | 1 | 13612ms |
| P8 全局设置 |  | 8/8 | 0 | 0 | 1 | 10174ms |

### 外壳（标题栏 / 左栏 / 应用菜单 / 日志抽屉 / 主题）

到达方式：instances　用例：`scripts/test/ui/shell.ps1`
...
```
pid=39752 hwnd=19007398 class=WinUIDesktopWin32WindowClass route=instances
...
到达方式：instances　用例：`scripts/test/ui/p1-instances.ps1`
...
```
pid=52944 hwnd=25954886 route=instances
```
...
到达方式：detail/first/plugins　用例：`scripts/test/ui/p2-detail-plugins.ps1`
...
```
pid=46060 hwnd=5048666 route=detail/first/plugins
```
...
到达方式：detail/first/settings　用例：`scripts/test/ui/p3-detail-settings.ps1`
...
```
pid=47572 hwnd=5705836 route=detail/first/settings
```
...
到达方式：detail/first/saves　用例：`scripts/test/ui/p4-detail-saves.ps1`
...
```
pid=50600 hwnd=8851168 route=detail/first/saves
```
...
到达方式：detail/first/logs　用例：`scripts/test/ui/p5-detail-logs.ps1`
...
```
pid=55700 hwnd=18941774 route=detail/first/logs
```
...
到达方式：engines　用例：`scripts/test/ui/p6-engines.ps1`
...
```
pid=51340 hwnd=23595454 route=engines
```
...
到达方式：create　用例：`scripts/test/ui/p7-wizard.ps1`
...
```
pid=9712 hwnd=23726526 launchRoute=create
...
到达方式：settings　用例：`scripts/test/ui/p8-settings.ps1`
...
```
pid=38932 hwnd=23792062 launchRoute=settings
global settings reached via deep link
```

</details>

## 6. unverifiable 清单（不计入通过）

本次运行没有 unverifiable 项。

## 7. 发现的真实缺陷

### DEFECT-1　深链 `WHALES_SMOKE_ROUTE=create|settings` 只换标题不换内容

- 严重度：高（会让所有依赖深链的自动化拿到空壳）
- 状态：已由并行的源码修复解决；本套用例保留 P7-08 / P8-08 两条断言专门盯住它
- 证据：
...
- 对本套用例的影响：P7 / P8 现在直接使用深链，并在深链失效时自动退回"点击真实入口"继续跑完其余断言；但 P7-08 / P8-08 会红，从而把回归暴露出来，而不是悄悄降级。

### DEFECT-2　`NavigateSmokeRoute` 缺少显式 `case "instances"`

- 严重度：低（当前不显形，但是静默失效的隐患）
- 状态：已修复
- 证据：
...
- 对本套用例的影响：无功能影响；属于"自动化依赖的行为没有显式契约"的清理。
...
- 对本套用例的影响：P3-02 只断言「行号列存在且可读」，不对行号数量做断言。
...
```json
{
  "environment": {
    ...
    "tmpHome": "F:\\Temp\\whales-ui-test-HcmzEQ"
  },
  "expectations": {
    "instanceCount": 3,
    "instanceNames": [
    ...
    "noEngineHomeDir": "F:\\Temp\\whales-ui-test-txnYwx",
    "emptyHomeDir": "F:\\Temp\\whales-ui-test-0PRyaa",
    "firstInstanceName": "UI Alpha",
    "firstInstanceDir": "ui-alpha",
    "firstInstanceId": "ui-fixture-0001",
    "sessionCount": 0,
    "searchHitQuery": "UI Alpha",
    "searchHitCount": 1,
    "noMatchQuery": "zzz-no-such-instance-zzz",
    "fixtureEngineVersion": "0.0.1-uitest"
  },
  "framework": {
